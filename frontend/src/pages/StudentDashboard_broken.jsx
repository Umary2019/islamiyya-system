...
const StudentDashboard = () => {
  ...
  const successRate = records.length > 0 
    ? ((records.length - records.filter(r => r.grade === 'F').length) / records.length * 100).toFixed(1)
    : 0;
  ...
  const getGradeColor = (grade) => {
    const colors = {
      'A+': 'bg-green-100 text-green-800', 'A': 'bg-green-100 text-green-800', 'A-': 'bg-green-50 text-green-700',
      'B+': 'bg-blue-100 text-blue-800', 'B': 'bg-blue-100 text-blue-800', 'B-': 'bg-blue-50 text-blue-700',
      'C+': 'bg-yellow-100 text-yellow-800', 'C': 'bg-yellow-100 text-yellow-800', 'C-': 'bg-yellow-50 text-yellow-700',
      ...
      {/* Stats Cards */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
            ...
            <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-yellow-500">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-4xl font-bold text-yellow-600">{totalRecords}</p>
                </div>
            ...
            <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-blue-500">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-gray-600 text-sm font-semibold mb-2">Average Grade</p>
                  <p className="text-4xl font-bold text-blue-600">{averageGrade}</p>
                </div>
                      <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-blue-500">
              </div>
                      <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-blue-500">
            ...
            <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-purple-500">
                      <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-blue-500">
                <div>
                  <p className="text-gray-600 text-sm font-semibold mb-2">Summaries</p>
                  <p className="text-4xl font-bold text-purple-600">{summaries.length}</p>
                </div>
                <div className="text-5xl opacity-20">📊</div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-lg p-8">
            {loading ? (
              ...
              <div>
                <h2 className="text-3xl font-bold text-blue-900 mb-8">Daily Recitation Records</h2>
                {records.length === 0 ? (
              ...
              <div>
                <h2 className="text-3xl font-bold text-blue-900 mb-8">Weekly Summaries</h2>
                {summaries.length === 0 ? (
                  <div className="text-center py-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {summaries.map((summary) => (
                      <div key={summary._id} className="border border-gray-200 p-6 rounded-lg hover:shadow-lg transition">
                        <div className="flex justify-between items-start mb-4">
                          <h3 className="text-lg font-bold text-blue-900">Week {summary.week}, {summary.year}</h3>
                          <span className="text-sm bg-blue-100 text-blue-800 px-3 py-1 rounded-full font-semibold">Submitted</span>
                        </div>
                        <div className="space-y-3">
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <p className="text-2xl font-bold text-green-600">{summary.totalPages}</p>
                          </div>
              ...
              <div>
                <h2 className="text-3xl font-bold text-blue-900 mb-8">Performance Analysis</h2>
                {records.length === 0 ? (
                  <div className="text-center py-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-gradient-to-br from-green-50 to-green-100 p-8 rounded-lg border-l-4 border-green-500">
                      <p className="text-gray-700 font-semibold text-sm mb-2">Success Rate</p>
                      <p className={`text-5xl font-bold ${getPerformanceColor()}`}>{successRate}%</p>
                      <p className="text-gray-600 text-xs mt-4">
                        {totalRecords - records.filter(r => r.grade === 'F').length} of {totalRecords} assessments passed
                      </p>
                    </div>

                    {/* Average Grade Card */}
                    <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-8 rounded-lg border-l-4 border-blue-500">
                      <p className="text-gray-700 font-semibold text-sm mb-2">Average Grade</p>
                      <p className="text-5xl font-bold text-blue-600">{averageGrade}</p>
                      <p className="text-gray-600 text-xs mt-4">Based on {totalRecords} records</p>
                    </div>

                    {/* Failed Count Card */}
                    <div className="bg-gradient-to-br from-red-50 to-red-100 p-8 rounded-lg border-l-4 border-red-500">
                      <p className="text-gray-700 font-semibold text-sm mb-2">Need Improvement</p>
                      <p className="text-5xl font-bold text-red-600">{records.filter(r => r.grade === 'F').length}</p>
